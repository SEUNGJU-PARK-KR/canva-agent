...
async function progress(text: string) {
  await appendFile(path.join(out, "progress.log"), `${new Date().toISOString()} ${text}\n`, "utf8");
}
async function dismiss(page: Page) {
  for (const name of [/모두 수락/i, /모두 거부/i, /동의/i, /Accept all/i, /Reject all/i, /I agree/i]) {
    try {
      const button = page.getByRole("button", { name }).first();
      if ((await button.count()) && (await button.isVisible({ timeout: 250 }))) {
...
async function forcePlay(page: Page) {
  ...
  for (const selector of ["button.ytp-large-play-button", "button.ytp-play-button", ".ytp-cued-thumbnail-overlay", "video"]) {
    try {
      ...
      if ((await node.count()) && (await node.isVisible({ timeout: 200 }))) await node.click({ force: true, timeout: 800 });
    } catch {}
  }
}
async function state(page: Page) {
  return page.evaluate(() => {
    ...
    return { found: true, currentTime: Number(v.currentTime || 0), duration: Number.isFinite(v.duration) ? Number(v.duration) : null,
      paused: v.paused, readyState: v.readyState, networkState: v.networkState, width: v.videoWidth || 0, height: v.videoHeight || 0,
...
async function shot(page: Page, file: string) {
  try {
    ...
    if ((await video.count()) && (await video.isVisible({ timeout: 200 }))) { await video.screenshot({ path: file }); return; }
  } catch {}
  await page.screenshot({ path: file, fullPage: false }).catch(() => undefined);
}

await mkdir(out, { recursive: true });
await progress(`worker start ${id}`);
const browser = await chromium.launch({ headless: false, args: ["--no-sandbox", "--disable-dev-shm-usage", "--autoplay-policy=no-user-gesture-required", "--disable-blink-features=AutomationControlled", "--window-size=1600,1000"] });
...
for (const [name, url] of variants) {
  ...
  page.on("response", (r) => { const u = r.url(); if (u.includes("googlevideo.com") || u.includes("videoplayback") || u.includes(".m3u8")) media.add(u); });
  const item: Record<string, any> = { name, url, states: [] };
  await progress(`${name} navigate begin`);
  try {
    try { await page.goto(url, { waitUntil: "commit", timeout: 25000 }); }
    catch (error) { item.navigationError = String(error); }
    ...
    for (let second = 1; second <= 10; second++) {
      ...
      if ([2, 5, 10].includes(second)) await shot(page, path.join(dir, `wall_${String(second).padStart(2, "0")}.png`));
      if (second === 5) await forcePlay(page);
    }
    item.states = states;
    ...
    item.success = Number(item.timeDelta) >= 4 && Number(item.maxPixels) >= 320 * 180;
  ...
  attempts.push(item);
  await progress(`${name} finished success=${Boolean(item.success)} delta=${item.timeDelta ?? 0}`);
  await page.close();
  if (item.success) break;
}
const result = { videoId: id, runtime: "CopilotKit/OpenBot agent-computer image", success: attempts.some((a) => a.success),
  successVariant: attempts.find((a) => a.success)?.name || null, attempts };
await saveJson(path.join(out, "summary.json"), result);
await progress(`worker finished success=${result.success}`);
