...
async def state(page) -> dict[str, Any]:
    try:
        return await page.evaluate("""() => { const v=document.querySelector('video'); if(!v)return {found:false}; return {found:true,currentTime:Number(v.currentTime||0),duration:Number.isFinite(v.duration)?Number(v.duration):null,paused:v.paused,readyState:v.readyState,networkState:v.networkState,width:v.videoWidth||0,height:v.videoHeight||0,error:v.error?{code:v.error.code,message:v.error.message||''}:null,src:v.currentSrc||v.src||''}; }""")
    except Exception as exc:
        return {"found": False, "error": f"{type(exc).__name__}: {exc}"}


async def force_play(page) -> None:
    try:
        await page.evaluate("""async () => { const v=document.querySelector('video'); if(v){v.muted=true;v.volume=0;try{await v.play()}catch(e){}} }""")
    except Exception:
        pass
            ...
            for name, template in VARIANTS:
                ...
                try:
                    ...
                    for i in range(1, 21):
                        ...
                        st = await state(page)
                        st["wall_seconds"] = round(i * 1.5, 1)
                        rec["states"].append(st)
                        if i in {1,2,3,5,8,12,16,20}:
                            target = d / f"frame_{i:02d}_{i*1.5:04.1f}s.png"
                            try:
